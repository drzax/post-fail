				...
				for (let line of lines) {
					...
					for (let block of street.b) {
						...
						let indexThisBlock =
							...
							numericWords.some((w) => w === block.n || w === street.p);
						if (indexThisBlock) {
							...
							for (let unitType in block.u) {
								...
								for (let unitNumber of unitNumbers) {
									if (Array.isArray(unitNumber) && unitNumber.length === 2) {
										...
										let indexThisUnit =
											indexThisBlock || numericWords.some((w) => w === unitNumber);
										if (indexThisUnit) {
											searchIndexDocuments[id] = {
												id: id,
...
const prettyAddress = (address: string): string => {
	return address.toUpperCase().replace(/(^|[^A-Z])(['A-Z]+)/g, (entireMatch, separator, word) => {
		let prettyWord = states.has(word)
			? word
...
export function geocode(input: string, limit: number): Promise<GeocodeResult> {
	return new Promise((resolve, reject) => {
		...
		let cleanedInput = input
			...
			.replace(/\s+/g, ' ')
			.toUpperCase();
		let words: NumericWord[] = cleanedInput
			.split(' ')
			.map((word) => (isFinite(+word) ? +word : word));
